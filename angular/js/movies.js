    ...
    .controller('MoviesController', function($scope, $http) {
        ...
        $http.get('data/movies-2014.min.json')
            .then(function(results) {
               $scope.movies = results.data.map(function(movie) { // map is a function on an array, similar to forEach
        ...
        $scope.setSort = function(propertyName) {
            if ($scope.sortCol === propertyName) { // === if the data types are different, it won't work
